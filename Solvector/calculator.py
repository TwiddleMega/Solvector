...
class Calculator:
    def __init__(self, master):
        ...
        for button in buttons:
            ...
            if button == '=':
                ...
                cmd = lambda x=0: self.status('factorial')
    ...
    def factorialnum(self, value):
        try:
            n=int(value)
            if n == 1:
                return n
            else:
                return n*self.factorialnum(n-1)
        except:
            self.status('error')
    ...
    def status(self, value):
        if value == 'clear':
            ...
            expression = self.display.get()
            expression = self.factorialnum(expression)
            self.status('clear')
            self.display.insert(0, str(expression))
